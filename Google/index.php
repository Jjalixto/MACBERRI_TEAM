...
if (isset($_GET["code"])) {
    ...
    $revisar = "SELECT * FROM users WHERE token = '$_SESSION[access_token]'";
    $resul = mysqli_query($conn, $revisar);
    if (mysqli_num_rows($resul) > 0) {
        echo "usuario ya existente";
    } else {
        //insertar datos 
        $datos = "INSERT INTO users(first_name, last_name,email,token) VALUES ('{$_SESSION['user_first_name']}','{$_SESSION['user_last_name']}','{$_SESSION['user_email_address']}','{$_SESSION['access_token']}')";
        $sql = mysqli_query($conn, $datos);
        echo "llegaste";
    }
}
